...
//        var_dump($request->domain());exit;
        for ($i = 0; $i < 5; $i++) {
            ...
            $addData['notify_url'] = "http://" . $request->domain() . "/api/ceshi/ordernotify";  //回调地址
            $addData['notify_url'] = "http://" . $request->domain() . "/api/ceshi/ordernotify";  //回调地址
            $addData['sign'] = md5($addData['write_off_sign'] . $addData['order_no'] . $addData['account'] . "jfkdakjfhamdfka29u9");
            $addOrderRes = curlPost("http://" . $request->domain() . "/api/orderhexiao/uploadorder", $addData);
            var_dump($addOrderRes);exit;
