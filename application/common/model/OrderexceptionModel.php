...
class OrderexceptionModel extends Model
{
    protected $table = 'bsa_order_exception';
     ...
     * @return array
     */
    public function addLog($user = "", $action = "", $addParam)
    {
        try {

//            $has = $this->where($where)->findOrEmpty()->toArray();
//            if (!empty($has)) {
//                return modelReMsg(-2, '', '此核记录已经存在');
//            }
            if (empty($addParam) || empty($action)) {
                return modelReMsg(-2, '', '记录内容不能为空！');
            }

            $addParam['action'] = $action;
            $addParam['user'] = $user;
            $addParam['time'] = date('Y-m-d H:i:s', time());
