...
//                ->where()
     ...
     * @return array
     */
    public function getRoleMenuMapByNodeUser($roleId, $nodeUser = "merchant")
    {
        try {
            $res = [];
            ...
            $roleInfo = $roleModel
                ->where('is_menu', 2)
                ->getRoleInfoById($roleId)['data'];
//                var_dump($roleInfo);
//                exit;
            if (!empty($roleInfo)) {
                $where['is_menu'] = 2;
                $where['node_user'] = $nodeUser;
                $res = $this->field('node_id as id,node_name as title,node_pid as pid,node_path,node_icon')
                    ->where($where)
                    ->whereIn('node_id', $roleInfo['role_node'])
                    ->select()->toArray();
            }
